...
class Customer_balance_lib extends Custom_Model {

    public function __construct($deleted=NULL)
    {
        $this->deleted = $deleted;
        $this->tableName = 'wallet_balances';
    ...
    function fill($cust,$month,$year,$begin=0,$end=0)
    {
       ...
       if ($num == 0)
       {
          $trans = array('customer' => $cust, 'month' => $month, 'year' => $year, 'beginning' => $begin, 'end' => $end, 'created' => date('Y-m-d H:i:s'));
    ...
    function fill_vamount($cust,$month,$year,$amt=0)
    {
       ...
       if ($num == 0)
       {
          $trans = array('customer' => $cust, 'month' => $month, 'year' => $year, 'vamount' => $amt);
